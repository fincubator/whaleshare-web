...
const VOTE_WEIGHT_DROPDOWN_THRESHOLD = 100.0;
  ...
  render() {
    ...
    if (showList && total_votes > 0 && active_votes) {
      ...
      for (let v = 0; v < avotes.length && voters.length < MAX_VOTES_DISPLAY; ++v) {
        ...
        voters.push({
          value: (sign > 0 ? '+ ' : '- ') + voter + ' (' + (percent / 100) + '%, ' + value_voter.toFixed(2) + ' WLS)',
          link: '/@' + voter
        })
      }
        ...
        <span className="Voting">
                  <span className="Voting__inner">
                      <span className={classUp}>
                          {votingUpActive ? sh : <a href="#" onClick={voteUpClick}
                                                    title={myVote > 0 ? tt('g.remove_vote') : tt('g.upvote')}>{sh}</a>}
                        {dropdown}
                      </span>
                    {payoutEl}
                  </span>
