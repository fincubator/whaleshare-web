export const routeRegex = {
  PostsIndex: /^\/(@[\w\.\d-]+)\/feed\/?$/,
  UserProfile1: /^\/(@[\w\.\d-]+)\/?$/,
  UserProfile2: /^\/(@[\w\.\d-]+)\/(blog|posts|comments|shares|recommended|transfers|curation-rewards|author-rewards|permissions|created|recent-replies|feed|password|followed|followers|settings)\/?$/,
  UserProfile3: /^\/(@[\w\.\d-]+)\/[\w\.\d-]+/,
  CategoryFilters: /^\/(hot|votes|responses|trending|trending30|cashout|payout|payout_comments|created|active)\/?$/ig,
...
export default function resolveRoute(path) {
  ...
  match = path.match(/^\/(hot|votes|responses|trending|trending30|cashout|payout|payout_comments|created|active)\/?$/)
    || path.match(/^\/(hot|votes|responses|trending|trending30|cashout|payout|payout_comments|created|active)\/([\w\d-]+)\/?$/);
  if (match) {
